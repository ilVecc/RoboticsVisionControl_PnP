...
if __name__ == '__main__':
    ...
    model = np.loadtxt('points.txt')
    points = model[:, 0:3]
    
    sub_points = points[np.random.choice(points.shape[0], 15 * points.shape[0] // 100, replace=False), :]

    print(sub_points.shape[0])
    print(points.shape[0])

    cube_fit = Cuboid()
    eqs, inliers = cube_fit.fit(sub_points, maxIteration=2000)
